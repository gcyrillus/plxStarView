 ...
 * */
	...
	if($this->getParam('stars')=='0') {$stars='☆';} else {$stars='★';} 
		...
		if (!array_key_exists($id,$jsonDatas)) {
			...
			$jsonDatas[$id]= array('nbvote'=>'0' , 'points'=>'0', 'average'=>'0', 'nbview' =>'0', 'ips'=> array('0.0.0.0'), 'title' => str_replace('-',' ',prev($title)) , 'rated' => '1'	);	
			...
			echo '
				<form style="--average:'.$jsonDatas[$id]['average'].'%" id="rate-'.$id.'" action="../../plugins/'. __CLASS__ .'/rateIt.php" method="post">
				  <fieldset>
					<legend class="rate-infos">Votes <sup>('.$jsonDatas[$id]['nbvote'].' / '.$jsonDatas[$id]['average'].'%)</sup></sup><sub>vue(s)'.$jsonDatas[$id]['nbview'].'</sub></legend>
